...
application.config['ZIP_FOLDER'] = ZIP_FOLDER
...
def download_file():
    files = os.listdir(application.config['OUTPUT_FOLDER'])
    print(f"Archivos en la carpeta de salida: {files}")
    return render_template('download.html', files=files)

@application.route('/download_zip')
def download_zip():
    zip_filename = os.path.join(application.config['ZIP_FOLDER'], 'output_files.zip')
    with zipfile.ZipFile(zip_filename, 'w') as zipf:
        for root, _, files in os.walk(application.config['OUTPUT_FOLDER']):
            for file in files:
                zipf.write(os.path.join(root, file), file)
    return send_from_directory(application.config['ZIP_FOLDER'], 'output_files.zip')

@application.route('/download/<filename>')
def download(filename):
    try:
        return send_from_directory(application.config['OUTPUT_FOLDER'], filename)
